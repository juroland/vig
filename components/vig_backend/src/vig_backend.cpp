...
BackendClient::send_heartbeat(const vig::telemetry::TelemetryData &telemetry) {
  ...
  vig::net::HttpClient client(api_base_url_ + "/api/devices/heartbeat", setup_token_);
...
Expected<void> BackendClient::send_offline() {
  ...
  vig::net::HttpClient client(api_base_url_ + "/api/devices/offline", setup_token_);
