    ...
    public:
        ...
        void run()
        {
            ...
            xTaskCreatePinnedToCore([](void *arg)
                                    { static_cast<Device *>(arg)->camera_task(); }, "camera_task", 16384, this, 5, nullptr, 1);
    ...
    private:
        ...
        void camera_task()
        {
            ...
            while (true)
            {
                ...
                if (!frame_res)
                {
                    ESP_LOGE(TAG, "Capture failed: %.*s", (int)to_string(frame_res.error()).size(), to_string(frame_res.error()).data());
                    vTaskDelay(pdMS_TO_TICKS(100)); // Cool down on error
                    continue;
                }

                uint64_t current_pts = esp_timer_get_time() / 1000;
                auto encoded_res = encoder_.encode(frame_res->data.data(), frame_res->data.size(), current_pts);

                if (!encoded_res)
                {
                    ESP_LOGE(TAG, "Encode failed: %.*s", (int)to_string(encoded_res.error()).size(), to_string(encoded_res.error()).data());
                }
                else
                {
...
extern "C" void app_main()
{
    xTaskCreatePinnedToCore([](void *arg)
        ...
        if (!start_res) {
            ESP_LOGE("Main", "Failed to start device: %.*s", (int)vig::to_string(start_res.error()).size(), vig::to_string(start_res.error()).data());
            vTaskDelete(nullptr);
            return;
        }

        device.run(); }, "main_app_task", 24576, nullptr, 5, nullptr, 0);
}
