...
Expected<void> H264Encoder::init(int width, int height, int fps, int bitrate_kbps,
                                 int gop) {
  if (is_initialized_)
    return {};
  ...
  if (out_buffer_.empty()) {
    ESP_LOGE(TAG, "Failed to allocate aligned output buffer (%zu bytes)",
             out_buffer_size_);
    esp_h264_enc_close(handle_);
    esp_h264_enc_del(handle_);
    handle_ = nullptr;
...
Expected<EncodedFrame> H264Encoder::encode(const uint8_t *yuv_data, size_t size,
                                           uint64_t pts) {
  if (!is_initialized_)
    return std::unexpected(DeviceError::EncoderInitFailed);
  ...
  EncodedFrame frame;
  frame.data.assign(out_frame.raw_data.buffer,
                    out_frame.raw_data.buffer + out_frame.length);
  frame.pts = out_frame.pts;
  frame.is_keyframe = (out_frame.frame_type == ESP_H264_FRAME_TYPE_IDR ||
