...
namespace vig::camera
{
    ...
    IRAM_ATTR static bool camera_trans_finished_cb(esp_cam_ctlr_handle_t handle, esp_cam_ctlr_trans_t *trans, void *user_data)
    {
    ...
    Expected<void> HardwareCamera::init()
    {
        ...
        {
            ESP_LOGW(TAG, "Failed to acquire LDO channel for MIPI PHY (might be already active)");
        }

        // Provide External Clock (XCLK)
        if (esp_cam_sensor_xclk_allocate(ESP_CAM_SENSOR_XCLK_ESP_CLOCK_ROUTER, &impl_->xclk_handle) != ESP_OK)
        {
        ...
        ESP_LOGI(TAG, "Sensor format set to: %s (%dx%d)", sensor_fmt.name, (int)sensor_fmt.width, (int)sensor_fmt.height);
        ...
        isp_cfg.input_data_color_type = (sensor_fmt.format == ESP_CAM_SENSOR_PIXFORMAT_RAW10) ? ISP_COLOR_RAW10 : ISP_COLOR_RAW8;
        ...
        if (esp_cam_ctlr_register_event_callbacks(impl_->csi_handle, &cbs, impl_->frame_ready_que) != ESP_OK)
        {
            return std::unexpected(DeviceError::CameraInitFailed);
        }
        ...
        {
            impl_->csi_buffers[i] = esp_cam_ctlr_alloc_buffer(impl_->csi_handle, impl_->csi_buffer_len, MALLOC_CAP_SPIRAM | MALLOC_CAP_DMA);
        ...
        int enable = 1;
        if (esp_cam_sensor_ioctl(impl_->camera_handle, ESP_CAM_SENSOR_IOC_S_STREAM, &enable) != ESP_OK)
        {
    ...
    {
        // Recycle the last transaction buffer (from the previous frame capture) back to the CSI controller.
        // This allows the camera/CSI/ISP to overwrite this buffer only after the host has finished encoding it!
        if (impl_->has_last_transaction)
        {
            esp_cache_msync(impl_->last_trans.buffer, impl_->last_trans.buflen, ESP_CACHE_MSYNC_FLAG_INVALIDATE);
        ...
        // Invalidate cache to ensure CPU reads fresh data directly from physical PSRAM DMA buffer
        esp_cache_msync(trans.buffer, trans.buflen, ESP_CACHE_MSYNC_FLAG_INVALIDATE);
        ...
        // Pure Zero-Copy: Wrap the DMA buffer address in our CameraFrameBuffer without copy!
        frame.data.set_external_buffer(static_cast<const uint8_t *>(trans.buffer), trans.received_size);

        // Store current transaction as last_trans to recycle it on the NEXT call to capture()
