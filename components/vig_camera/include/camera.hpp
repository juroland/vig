#ifndef VIG_CAMERA_CAMERA_HPP
#define VIG_CAMERA_CAMERA_HPP
...
namespace vig::camera
{
     ...
     * 1. **Internal Pool Mode (Owning)**: Reuses a single, static, globally shared pool
     *    pre-allocated in PSRAM (using 64-byte alignment to support P4 cache sync / DMA
     *    transfers). This mode is used when generating mock frames or performing
    ...
    public:
        ...
        static constexpr size_t MAX_RESOLUTION_WIDTH = 1920;
        static constexpr size_t MAX_RESOLUTION_HEIGHT = 1080;
         ...
         * hardware driver, * and the internal pool is a static global vector persistent
         ...
         * Lazily allocates the static buffer using the 64-byte aligned PSRAM allocator
        ...
        void ensure_internal_pool()
        {
            ...
            {
                static std::vector<uint8_t, memory::AlignedPsramAllocator<uint8_t>> global_psram_pool(kGlobalPoolSize);
