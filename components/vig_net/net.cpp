...
namespace vig::net
{
    ...
    Expected<std::string> HttpClient::post_json_with_response(const std::string &payload)
    {
        ...
        int read = esp_http_client_read_response(client_handle_, response.data(), content_length);
        if (read < 0)
        {
    ...
    Expected<void> NetworkManager::init_ethernet()
    {
        ...
        ESP_ERROR_CHECK(esp_event_handler_instance_register(ETH_EVENT, ESP_EVENT_ANY_ID, &eth_event_handler, this, nullptr));
        ESP_ERROR_CHECK(esp_event_handler_instance_register(IP_EVENT, IP_EVENT_ETH_GOT_IP, &eth_event_handler, this, nullptr));
    ...
    void NetworkManager::eth_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
    {
